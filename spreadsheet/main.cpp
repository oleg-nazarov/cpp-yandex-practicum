...
double ExecuteASTFormula(const std::string& expression) {
    ...
    return ParseFormulaAST(expression).Execute(*sheet);
}

std::variant<double, FormulaError> ExecuteFormula(const std::string& expression) {
    auto sheet = CreateSheet();
    return ParseFormula(expression)->Evaluate(*sheet);
}
...
void TestFormulaReferencedCells() {
...

    {
        ...
        ASSERT_EQUAL(std::get<double>(sheet->GetCell("A1"_pos)->GetValue()), 0);

        sheet->SetCell("A2"_pos, "5");
        ASSERT_EQUAL(std::get<double>(sheet->GetCell("A1"_pos)->GetValue()), 5);
...

    {
        ...
        sheet->SetCell("A1"_pos, "=A2+10");
        ASSERT_EQUAL(std::get<double>(sheet->GetCell("A1"_pos)->GetValue()), 10);

        sheet->SetCell("A3"_pos, "5");
        ASSERT_EQUAL(std::get<double>(sheet->GetCell("A1"_pos)->GetValue()), 15);

        sheet->ClearCell("A3"_pos);
        ASSERT_EQUAL(std::get<double>(sheet->GetCell("A1"_pos)->GetValue()), 10);

        sheet->SetCell("A2"_pos, "7");
        sheet->SetCell("A3"_pos, "8");

        ASSERT_EQUAL(std::get<std::string>(sheet->GetCell("A2"_pos)->GetValue()), "7");
        ASSERT_EQUAL(std::get<double>(sheet->GetCell("A1"_pos)->GetValue()), 17);

        sheet->SetCell("A2"_pos, "=A3+2");
        ASSERT_EQUAL(std::get<double>(sheet->GetCell("A2"_pos)->GetValue()), 10);
        ASSERT_EQUAL(std::get<double>(sheet->GetCell("A1"_pos)->GetValue()), 20);
    }
...
int main() {
    ...
    try {
        ...
        std::cout << fe.ToString() << " OK"s << std::endl;
    }

    // ParseFormula
    ASSERT_EQUAL(std::get<double>(ExecuteFormula("1/1")), 1.0);
    try {
        std::cout << std::get<FormulaError>(ExecuteFormula("1/0")) << " OK"s << std::endl;
