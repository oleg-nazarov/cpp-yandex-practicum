...
class NewInstance : public Statement {
   ...
   private:
    ...
    std::vector<std::unique_ptr<Statement>> init_args_;
};

// Базовый класс для унарных операций
class UnaryOperation : public Statement {
   public:
    explicit UnaryOperation(std::unique_ptr<Statement> arg) : arg_(move(arg)) {}
...
class BinaryOperation : public Statement {
   ...
   protected:
    std::unique_ptr<Statement> lhs_arg_;
    std::unique_ptr<Statement> rhs_arg_;
};
...
class Compound : public Statement {
   public:
    ...
    explicit Compound(Args&&... args) {
        if (sizeof...(args)) {
            (..., statements_.push_back(std::move(args)));
        }
    }
