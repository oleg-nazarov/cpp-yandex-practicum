...
void TestStrings() {
    {
        istringstream input(
            R"('word' "two words" 'double quote " inside' 'two double quotes "inside"' "single quote ' inside" "two single quotes 'inside'")"s);
        Lexer lexer(input);
        ...
        ASSERT_EQUAL(lexer.NextToken(), Token(token_type::String{"double quote \" inside"s}));
        ASSERT_EQUAL(lexer.NextToken(), Token(token_type::String{"two double quotes \"inside\""s}));
        ASSERT_EQUAL(lexer.NextToken(), Token(token_type::String{"single quote ' inside"s}));
        ASSERT_EQUAL(lexer.NextToken(), Token(token_type::String{"two single quotes 'inside'"s}));
    }

    {
        istringstream input(R"('one \' single' 'two \'single\'' "one \" double" "two \"double\"")");
        Lexer lexer(input);

        ASSERT_EQUAL(lexer.CurrentToken(), Token(token_type::String{"one ' single"s}));
        ASSERT_EQUAL(lexer.NextToken(), Token(token_type::String{"two 'single'"s}));
        ASSERT_EQUAL(lexer.NextToken(), Token(token_type::String{"one \" double"s}));
        ASSERT_EQUAL(lexer.NextToken(), Token(token_type::String{"two \"double\""s}));
    }
}
